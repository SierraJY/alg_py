...
# how
# 대기 시간이 짧아야 하는 것이 관건 (그리디 문제가 아님, 미래의 선택도 고려해야하기 때문)
...
# 주의사항
# 1 : 하드디스크가 작업을 수행하고 있지 않을 때에는 먼저 요청이 들어온 작업부터 처리해야함
# 여기에서 주의할 것은 jobs에서의 순서가 요청순서가 아니라 요청시간이 요청 순서가됨
# 2: 태스크를 처리하고 있는 중에도 새로운 태스크가 힙으로 들어올 수 있다!!!
